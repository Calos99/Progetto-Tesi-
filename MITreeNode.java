...
public class MITreeNode implements Comparable<MITreeNode> {
	...
	*/
	...
	/**
	*	This is a reference to the list of transactions contained in the node.
	*/
	...
	*/
	public MITreeNode(Interval MBI, byte[] hash, List<Transaction> data,
	...
	*	@param ts the list of transactions
	*	@return a leaf MI-tree node
	*/
	public static MITreeNode nodeFromTransactions(List<Transaction> ts) {
		return new MITreeNode(Interval.MBI(ts), Hash.hashTransactions(ts),
		ts, null);
	}
	...
	*/
	public static MITreeNode nodeFromChildren(List<MITreeNode> children) {
		...
		for (MITreeNode temp : children) {
			tl.addAll(temp.getData());
		}
