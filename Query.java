...
public final class Query {
  ...
  */
  public static List<Integer> filter(List<Integer> pts, Interval query) {
    ...
    while (!s.isEmpty()) {
      ...
      // Otherwise we have reached a leaf or pruned node.
        ...
        if (curr instanceof VLeaf) {
          List<Integer> records = ((VLeaf) curr).getRecords();
          partial = new VResult(records, Interval.MBI(records),
          Hash.hashIntegers(records));
        }
        // Otherwise it is a pruned node...
        else {
          VPruned pr = ((VPruned) curr);
          partial = new VResult(new ArrayList<Integer>(), pr.getMBI(),
          pr.getHash());
        }
        if (parent != null) content.get(parent).add(partial);
        else result = partial;
  ...
  */
  public static List<Integer> extract(VObject vo) {
