...
public final class MITree{
  ...
  * @param ts list of transactions
  * @param c page capacity
  * @return the root node of the constructed MI-tree index
  */
  public static MITreeNode buildPacked(List<Transaction> ts, int c) {
    ...
    List<List<Transaction>> leaves = Utility.split(ts, c);
    for (List<Transaction> ck : leaves) current.add(MITreeNode.nodeFromTransactions(ck)); 
    // Start merging.
