...
public final class Hash {
	...
	*/
	/*public static byte[] hashInteger(int p) {	//non serve
		try {
	...
	*/
	public static byte[] hashTransactions(List<Transaction> ts) {
		try {
			MessageDigest digest = MessageDigest.getInstance("SHA-256");
			ByteBuffer bb = ByteBuffer.allocate(24 * ts.size());
			for (Transaction p : ts) bb.putLong(p.getID()).putLong(p.getAddressID()).putLong(p.getAmount());						//da cambiare con le triple per i campi
			return digest.digest(bb.array());
		}
		catch (Exception e) {
			System.err.println("Something went wrong while hashing transactions!");
			e.printStackTrace();
			return null;
		}
	}
	...
	*/
	/*public static byte[] hashInterval(Interval i) {	//non serve per ora
		try {
			MessageDigest digest = MessageDigest.getInstance("SHA-256");
	...
	public static byte[] hashNode(List<MITreeNode> children) {
		try {
			...
			for (MITreeNode c : children) {
				Interval i = c.getMBI();
				byte[] rbuf = ByteBuffer.allocate(16).putLong(i.min)
				.putLong(i.max).array();
				strm.write(rbuf);
	...
	*/
	/*public static byte[] hashBlock(Block b) {			//questa non serve
		try {
			MessageDigest digest = MessageDigest.getInstance("SHA-256");
			ByteArrayOutputStream strm = new ByteArrayOutputStream();
	...
	public static byte[] reconstruct(List<Interval> intervals, List<byte[]> hashes)
	{
		...
		try {
			...
			for (int i = 0; i < intervals.size(); i++) {
				...
				byte[] rbuf = ByteBuffer.allocate(16).putLong(in.min).
				putLong(in.max).array();
				strm.write(rbuf);
